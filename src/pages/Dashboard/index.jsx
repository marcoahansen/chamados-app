...
import { getFirestore, setDoc, getDoc, updateDoc, addDoc, collection, getDocs, query, orderBy, limit, documentId, startAfter } from 'firebase/firestore';
...
function DashBoard() {
  ...
  function handleMore(){
    setLoadMore(true)
    getDocs(query(chamadosRef, orderBy('created', 'desc'), startAfter(lastDocs), limit(5)))
    .then((snapshot)=>{
      updateState(snapshot)
    })
    .catch((error)=>{
      ...
      <Paper sx={{ p: 2, display: 'flex', justifyContent:'space-between', alignItems:'center'}}>
          ...
          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems:'flex-end' }}>
            ...
            <TableContainer component={Paper}>
              <Table sx={{mt: 2, minWidth: 500 }} aria-label="customized table">
                <TableHead>
                  <TableRow>
